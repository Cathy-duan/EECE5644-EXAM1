...
mu(:,1) = [15;1]; mu(:,2) = [17;5];
Sigma(:,:,1) = [6 2;2 5]; Sigma(:,:,2) = [4 -1;-1 14];
p0 = [0.4 0.6]; % probability split between 2 distributions

% Class 1 parameters (2 gaussians)
mu(:,3) = [6;2]; mu(:,4) = [10;4]; 
Sigma(:,:,3) = [4 -1;-1 16]; Sigma(:,:,4) = [14 2;2 4];
p1 = [0.3 0.7]; % probability split between 2 distributions

p = [0.2,0.8]; % class priors for labels 0 and 1 respectively
label = rand(1,N) >= p(1);
